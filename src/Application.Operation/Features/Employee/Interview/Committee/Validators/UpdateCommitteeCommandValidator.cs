...
public sealed class UpdateCommitteeCommandValidator : AbstractValidator<UpdateCommitteeCommand>
{
    public UpdateCommitteeCommandValidator()
    {
        ...
        RuleFor(x => x.Members)
            .Must(members => members.Count >= CommitteeConstants.MinimumCommitteeMembers)
            .WithMessage($"A committee must have at least {CommitteeConstants.MinimumCommitteeMembers} members.");
        RuleFor(x => x.Members)
