...
public sealed class CancelScheduleCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<CancelScheduleCommand, IResult<Unit>>
{
    public async Task<IResult<Unit>> Handle(CancelScheduleCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = schedule.Cancel(request.Reason);
        if (result.IsFailed)
        ...
        await unitOfWork.GetEntityRepository<InterviewAuditLog>().AddAsync(new InterviewAuditLog
        {
            ...
            Action = InterviewScheduleAuditActions.Cancelled,
