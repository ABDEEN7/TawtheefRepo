...
namespace Application.Operation.Features.Employee.JobManagement.JobOperations.Extensions;
    ...
    extension(IQueryable<jobEntity> query)
    {
        public IQueryable<jobEntity> ApplySorting(PaginatedRequest? pagination)
        {
            ...
            return sortBy switch
            {
                ...
                "status" => isDescending ? query.OrderByDescending(j => j.JobStatus != null ? j.JobStatus.NameEn : string.Empty) : query.OrderBy(j => j.JobStatus != null ? j.JobStatus.NameEn : string.Empty),
                "created" or "createddate" => isDescending ? 
                    query.OrderByDescending(j => j.CreatedBy!.FullNameEn).ThenByDescending(j => j.CreatedDate) : 
                    query.OrderBy(j => j.CreatedBy!.FullNameEn).ThenBy(j => j.CreatedDate),
                "updated" or "updateddate" => isDescending ? 
                    query.OrderByDescending(j => j.CreatedBy!.FullNameEn).ThenByDescending(j => j.UpdatedDate) : 
                    query.OrderBy(j => j.CreatedBy!.FullNameEn).ThenBy(j => j.UpdatedDate),
