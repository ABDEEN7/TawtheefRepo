    ...
    where TRequest : BaseSearchQuery, IRequest<IResult<List<DropdownOptions>>>
{
    ...
    public async Task<IResult<List<DropdownOptions>>> Handle(TRequest request, CancellationToken cancellationToken)
    {
        ...
        var data = appConfig.Value.EnableLookupsCaching 
            ? await cache.GetOrCreateAsync(cacheKey, async entry =>
                {
                    ...
                    return await FetchDataAsync(dbSet, languageToken, isPaged, request, cancellationToken);
                })
            : await FetchDataAsync(dbSet, languageToken, isPaged, request, cancellationToken);
        ...
        if (request is { Id: not null })
        {
            ...
            var mappedByIdList = mappedById.Select(x => new DropdownOptions
            {
                     ...
                     AdditionalData = new { x.NameAr, x.NameEn, x.DescriptionAr, x.DescriptionEn }
            }).ToList();
    ...
    private async Task<List<DropdownOptions>> FetchDataAsync(IQueryable<TLookup> dbSet, string languageToken, bool isPaged, TRequest request, CancellationToken cancellationToken)
    {
        var isArabic = languageToken == "ar";
