...
public class UpdateJobCommandValidator : AbstractValidator<UpdateJobCommand>
{
    public UpdateJobCommandValidator()
    {
        // Job ID Validation
        RuleFor(x => x.Job.Id)
            .NotEmpty().WithMessage(JobValidationMessages.JOB_ID_REQUIRED);
        ...
        When(x => x.Job != null, () =>
        {
            ...
            RuleFor(x => x.Job.Description)
                .NotEmpty().WithMessage(JobValidationMessages.JOB_DESCRIPTION_REQUIRED)
                .MaximumLength(2000).WithMessage(JobValidationMessages.JOB_DESCRIPTION_MAX_LENGTH);
                ...
                RuleForEach(x => x.Job.Quota.ResidentsBreakdowns)
                    .ChildRules(b =>
                    {
                        ...
                        b.RuleFor(rb => rb.NationalityId)
                            .NotEmpty().WithMessage(JobValidationMessages.QUOTA_NATIONALITY_REQUIRED);
                    });
            });
    ...
    private static bool HaveValidQuotaTotal(JobQuotaDto quotas)
    {
        ...
        if (Math.Abs(mainQuotasSum - 100) > 0.01m) // Allow small rounding error
            return false;

        // Check residents breakdown sum
        if (quotas.ResidentsBreakdowns == null || !quotas.ResidentsBreakdowns.Any())
            return false;

        var breakdownSum = quotas.ResidentsBreakdowns.Sum(rb => rb.Percentage);
