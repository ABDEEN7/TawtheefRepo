...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, FileUpload, TranslatePipe, DatePicker, NgIf, Button, InputText, NgClass],
...
export class ExperienceModal implements OnInit {
  ...
  // clamp to today for both pickers
  today = new Date();
  ...
  ngOnInit(): void {
    ...
    // Initialize disabled state of 'to' based on current
    this.syncToDisabled();
  }
  ...
  private syncToDisabled() {
    if (this.f['current'].value) {
      // disable and clear 'to' when currently working
  ...
  onSave() {
    ...
    const v = this.form.getRawValue(); // getRawValue to include disabled 'to'
    const payload = {
      org: v.org,
      title: v.title,
      from: v.from,
...
/** 'to' is required when 'current' is false */
...
/** Disallow future dates for the provided keys */
