...
export class Dashboard implements OnInit {
  @ViewChild('mainCandidatesChart', { read: ElementRef }) private mainCandidatesChart?: ElementRef<HTMLElement>;
  @ViewChild('mainJobsChart', { read: ElementRef }) private mainJobsChart?: ElementRef<HTMLElement>;
  @ViewChild('mainEmployeesChart', { read: ElementRef }) private mainEmployeesChart?: ElementRef<HTMLElement>;
  @ViewChild('mainInvitationsChart', { read: ElementRef }) private mainInvitationsChart?: ElementRef<HTMLElement>;
  ...
  private readonly quickActionDefinitions: QuickAction[] = [
    ...
    { key: 'sendInvitation', labelKey: 'dashboard.quickActions.sendInvitation', icon: 'hgi-sent', route: routes.portal.jobInvitationSummary, permission: Permissions.JobInvitations.View },
    { key: 'approveProfiles', labelKey: 'dashboard.quickActions.approveProfiles', icon: 'hgi-file-validation', route: routes.portal.approvalProfile, permission: Permissions.ProfileApproval.View },
    { key: 'manageUsers', labelKey: 'dashboard.quickActions.manageUsers', icon: 'hgi-settings-02', route: routes.portal.usersManagement, permission: Permissions.Users.Manage }
  ];
  readonly quickActions = computed(() => this.quickActionDefinitions.filter(action => this.auth.hasPermission(action.permission, action.requireAll ?? false)));
  ...
  private chartData(items: DashboardChartExportItem[]): ChartData<'doughnut'> { this.languageChange(); const total = items.reduce((sum, item) => sum + item.count, 0);
    return total === 0 ? { labels: [this.translate.instant('common.chart.noData')], datasets: [{ data: [1], backgroundColor: ['#E5E7EB'], borderWidth: 0 }] }
  ...
  private localized(items: { labelKey: string; count: number; color: string }[]): DashboardChartExportItem[] { this.languageChange(); return items.map(item => ({ ...item, label: this.translate.instant(item.labelKey) })); }
  ...
  private defaultKpis(): DashboardKpis { return { totalEmployees: 0, activeEmployees: 0, totalProfiles: 0, newProfilesToday: 0, newProfilesThisWeek: 0,
    ...
    completedTasks: 0, remainingTasks: 0, overdueTasks: 0, unassignedProfiles: 0, followedMinisterOfficeCandidates: 0 }; }
  private defaultJobKpis(): JobKpis { return { totalJobs: 0, draftJobs: 0, activeJobs: 0, pendingReviewJobs: 0, approvedJobs: 0, rejectedJobs: 0,
    newJobsToday: 0, pendingPointConfigurationJobs: 0, needPointUpdateJobs: 0, pendingPointApprovalJobs: 0, needUpdateJobs: 0,
    readyForAnnouncementJobs: 0, publishedJobs: 0, closedJobs: 0, cancelledJobs: 0 }; }
}
