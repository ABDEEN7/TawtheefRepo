...
import { employeeAssignmentTranslationKey, TeamPerformanceRow } from '../../../models/dashboard-employees.model';
...
@Component({ selector: 'app-dashboard-employees-dialog', standalone: true,
  imports: [CommonModule, ChartModule, TableModule, TranslatePipe, I18nNamespaceDirective],
  templateUrl: './employees-dialog.html', styleUrl: './employees-dialog.scss', changeDetection: ChangeDetectionStrategy.OnPush })
export class EmployeesDialog implements OnInit {
  ...
  readonly tableFilters = signal<OperationsDashboardFilters>({ pageNumber: 1, pageSize: 10, search: '', sortBy: '', sortDirection: 'asc' });
  ...
  ngOnInit(): void { this.loadTeamPerformance(); }
  loadTeamPerformance(): void { this.api.getTeamPerformance({ ...this.filters(), ...this.tableFilters() })
    .subscribe(result => this.teamPerformance.set(result)); }
  exportList(): void { if (!this.canExport() || this.exportInProgress()) return; this.exportInProgress.set(true);
    ...
    this.api.exportList('Employees', { ...this.filters(), search: tableFilters.search,
      sortBy: tableFilters.sortBy, sortDirection: tableFilters.sortDirection }).pipe(finalize(() => this.exportInProgress.set(false)))
